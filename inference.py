...
def load_model():
    ...
    model.load_state_dict(
        torch.load("models/large-aging-model.h5", map_location=device)
    )
    model.to(device)
...
def add_center_back_on(
    original_frame, processed_square, feather_radius=10, corner_radius=50, margin=20
):
...
def process_image(image, input_age, output_age, frame_idx):
    ...
    blended_image = np.clip(
        (processed_output + np.array(resized_image)), 0, 255
    ).astype("uint8")
...
def process_video(
    input_video_path, input_age, output_age, output_video_path, progressive_aging=True
):
        ...
        for idx in range(frame_count):
            ...
            if idx == 0:
                ...
                output_video = cv2.VideoWriter(
                    output_video_path, cv2.VideoWriter_fourcc(*"mp4v"), fps, image.size
                )

            # Calculate the interpolated age for the current frame
            if progressive_aging:
                frame_age = input_age + (output_age - input_age) * (
                    idx / (frame_count - 1)
                )
            else:
                frame_age = (
                    output_age  # Fixed age for all frames if not progressive aging
                )
            ...
            output_video.write(
                cv2.cvtColor(np.array(processed_frame), cv2.COLOR_RGBA2BGR)
            )
            pbar.update(1)
            elapsed_time = time.time() - start_time
            estimated_remaining_time = (elapsed_time / (idx + 1)) * (
                frame_count - (idx + 1)
            )
